  ...
  (class object%
    ...
    (define segments-list
      (let* ((segments (cdr (assoc 'segment track-info)))
        ...
        (map (λ (id in out)
               (cons id (make-object segment% id track in out 'free)))
    ...
    (define detects-list
      (let* ((detects (cdr (assoc 'detection-block track-info)))
        ...
        (map (λ (id in out)
               (cons id (make-object detection-block% id track in out 'free)))
    ...
    (define switches-list
      (let* ((switches (cdr (assoc 'switch track-info)))
             ...
             (switch-outs (map (λ (a d) (cons a d))
                               ...
                               (map cadddr switches))))
        (map (λ (id in outs)
               (cons id (make-object switch% id track in outs 'free 'left)))
    ...
    (define crossings-list
      (let* ((crossings (cdr (assoc 'crossing track-info)))
        ...
        (map (λ (id segments)
               (cons id (make-object crossing% id track segments 'open)))
    ...
    (define lights-list
      (let* ((lights (cdr (assoc 'light track-info)))
        ...
        (map (λ (id segment)
               (cons id (make-object light% id track segment 'Hp1)))
